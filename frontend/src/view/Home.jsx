...
function Home() {
  ...
  const genreName = useMemo(() => {
    if (!genre || genres.length === 0) return "";
    const foundGenre = genres.find((g) => g.id === Number.parseInt(genre, 10));
    return foundGenre?.name || "";
  }, [genre, genres]);

  // Construire l'URL API dynamiquement selon le type
  const discoverEndpoint =
    contentType === "tv" ? "discover/tv" : "discover/movie";
  let apiUrl = `/api/tmdb/${discoverEndpoint}?sort_by=${sortBy}&order=${sortOrder}&page=${currentPage}`;
  if (genre) {
      ...
      <div className="flex justify-center items-center min-h-96">
        <div className="text-center">
          ...
          <p className="mt-4 text-slate-400">
            Chargement des {typeLabelSingular}...
          </p>
        </div>
      </div>
    );
