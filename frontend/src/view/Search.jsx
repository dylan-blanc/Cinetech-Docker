    ...
    const fetchTotals = async () => {
      ...
      try {
        const [movieRes, tvRes] = await Promise.all([
          fetch(
            `/api/tmdb/search/movie?query=${encodeURIComponent(query)}&page=1`,
          ),
          fetch(
            `/api/tmdb/search/tv?query=${encodeURIComponent(query)}&page=1`,
          ),
        ]);
    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/tmdb/${endpoint}?query=${encodeURIComponent(query)}&page=${currentPage}`,
        );
    ...
    <div className=''>
      ...
      <div className='mb-6'>
        ...
        <p className='text-slate-400 mt-2'>
          {totalMovies + totalTvShows} résultat
          {totalMovies + totalTvShows > 1 ? 's' : ''} trouvé
          {totalMovies + totalTvShows > 1 ? 's' : ''} au total
        </p>
      </div>
      ...
      {results.length === 0 ? (
        <div className='flex flex-col items-center justify-center min-h-64'>
          ...
          <p className='text-slate-400 text-center'>
            Aucun{mediaType === 'movie' ? ' film' : 'e série'} trouvé
            {mediaType === 'tv' ? 'e' : ''} pour '
